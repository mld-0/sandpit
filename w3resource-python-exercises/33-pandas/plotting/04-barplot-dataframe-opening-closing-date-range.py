...
_df_plot.plot(kind='bar', ax=ax)

ax.set_xticklabels([x.strftime("%Y-%m-%d") for x in _df_plot.index])  # Set x-axis date label format (so that time is not included if index consists of datetimes instead of dates)
