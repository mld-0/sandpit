...
for loop_i, (loop_species, loop_color) in enumerate(zip(_iris_species, _colors)):
    ...
    g = sns.jointplot(x='sepal_length', y='sepal_width', data=df_plot, kind='kde', fill=True, color=loop_color)
    g.plot_joint(plt.scatter, c="w", s=40, linewidth=1, marker="+")
    #g.ax_joint.collections[0].set_alpha(0)  # included in w3resource solution, (apparently) not needed?
    g.savefig(_saved_figures_sepal[-1])
    plt.close()
    g = sns.jointplot(x='petal_length', y='petal_width', data=df_plot, kind='kde', fill=True, color=loop_color)
    g.plot_joint(plt.scatter, c="w", s=40, linewidth=1, marker="+")
