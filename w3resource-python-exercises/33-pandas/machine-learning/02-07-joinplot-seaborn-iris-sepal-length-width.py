...
for loop_i, (loop_species, loop_color) in enumerate(zip(_iris_species, _colors)):
    loop_save_path_sepal = os.path.join(_temp_dir, "jointplot.sepal." + str(loop_i) + ".png")
    _saved_figures_sepal.append(loop_save_path_sepal)
    loop_save_path_petal = os.path.join(_temp_dir, "jointplot.petal." + str(loop_i) + ".png")
    _saved_figures_petal.append(loop_save_path_petal)
    loop_query = 'species=="%s"' % loop_species
    df_plot = df_iris.query(loop_query).drop(['species'], axis=1)
    g = sns.jointplot(x='sepal_length', y='sepal_width', data=df_plot, color=loop_color)
    g.savefig(loop_save_path_sepal)
    plt.close()
    g = sns.jointplot(x='petal_length', y='petal_width', data=df_plot, color=loop_color)
    g.savefig(loop_save_path_petal)
    plt.close()
