...
for loop_i, (loop_species, loop_color) in enumerate(zip(_iris_species, _colors)):
    ...
    g = sns.jointplot(x='sepal_length', y='sepal_width', kind='hex', data=df_plot, color=loop_color)
    g.savefig(_saved_figures_sepal[-1])
    plt.close()
    g = sns.jointplot(x='petal_length', y='petal_width', kind='hex', data=df_plot, color=loop_color)
